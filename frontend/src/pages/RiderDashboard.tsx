...
import RiderOrderMap from "../components/RiderOrderMap.tsx";
...
const RiderDashboard = () => {
  ...
  // ============================================================
// ✅ NEW ADDITION (SAFE - DOES NOT TOUCH EXISTING CODE)
// ============================================================
useEffect(() => {
  const handler = (event: any) => {
    const { orderId } = event.detail;

    console.log("🟢 (Fallback) Order received in UI:", event.detail);
  ...
  window.addEventListener("new-order-available", handler);

  return () => {
    window.removeEventListener("new-order-available", handler);
  };
}, [audioUnlocked]);
// ============================================================
  ...
  const handleSubmit = async () => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      try {
        ...
        // 1. Log the full error for you to debug in the terminal
        console.error("Rider Profile Submission Error:", error);

        // 2. Extract the best possible message
        const errorMessage =
          error.response?.data?.message || // Message from your backend logic
          error.response?.statusText ||    // "Internal Server Error" (from the browser)
          "Server connection failed. Please try again."; // Final fallback
